package com.taike.module_arcface.arcface.camera;
...
public class CameraHelper implements Camera.PreviewCallback {
    ...
    public void start() {
        synchronized (this) {
            ...
            //XLog.d("Camera.getNumberOfCameras   " + Camera.getNumberOfCameras());
            //XLog.d("Camera.CameraInfo.CAMERA_FACING_BACK   " + Camera.CameraInfo.CAMERA_FACING_BACK);
         /*   int numberOfCameras = Camera.getNumberOfCameras();
            for (int i = 0; i < numberOfCameras; i++) {
                Camera.CameraInfo info = new Camera.CameraInfo();
                Camera.getCameraInfo(i, info);
                XLog.d("mCamera1   info  " + GsonUtils.getGson().toJson(info));
                if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                    XLog.d("mCamera1   CAMERA_FACING_FRONT   ");
                }else if(info.facing == Camera.CameraInfo.CAMERA_FACING_BACK)
                {
                    XLog.d("mCamera1   CAMERA_FACING_BACK   ");
                }else
                {
                    XLog.d("mCamera1   CAMERA_FACING_OTHER   ");
                }
            }
